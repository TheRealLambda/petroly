...
export default function ChooseListMenu({ closeMenu, tasksLists, setCollection }) {
            ...
            <div className="section">
              ...
              <div className="collections_list">
                {list.tasksCollections.map(collection => {
                  return (
                    <div onClick={()=>setCollection(collection._id)} className="collection text-16-medium color-accent-80 bgcolor-BG button_effect_1_dark">{collection.name}</div>
                  )
                })}
              </div>
            </div>
